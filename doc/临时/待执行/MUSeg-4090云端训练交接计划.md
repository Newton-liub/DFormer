...
> 文档状态：待执行计划入口
>
> 当前只编写计划；未连接云端、未运行 GPU、未生成开发划分、未修改训练代码、未启动任何训练。

## 1. 当前唯一执行入口

后续新对话或执行模型必须从以下文件开始完整阅读：

`doc/临时/待执行/MUSeg阶段二长程Baseline与MVE/00-总索引与执行门禁.md`

然后严格按 `01` 至 `11` 的顺序和门禁执行。任何前置计划未验收时，不得提前进入后续文件。
...
用户后续将这些 `.md` 文件交给执行模型。在用户明确指定开始执行某个子计划前：

- 不生成新 split；
- 不改训练或模型代码；
- 不安装依赖或连接云端；
- 不运行 preflight、probe、训练、A2 或 B2；
- 不创建计划外提交。

本文件只承担总入口和旧计划作废声明，具体步骤以新目录中的 `00-总索引与执行门禁.md` 及对应子计划为准。
