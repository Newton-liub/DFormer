# OpenList 云服务器配置与可视化下载指南

本文用于在 CompShare 容器实例中部署 OpenList，挂载夸克网盘，并通过 OpenList 网页将文件复制到云服务器本地目录。文件复制任务在云服务器内执行，不经过本地电脑；任务进度可在 OpenList 的“任务”页面查看。
...
使用容器实例，并确保实例映射了 HTTP `5244` 端口。当前实例的容器入口不是 systemd，因此 OpenList 使用 `/start.d` 脚本自启动。
...
这是推荐的下载方式。文件不会下载到本地 Windows 电脑，而是在云服务器内部执行复制任务。
