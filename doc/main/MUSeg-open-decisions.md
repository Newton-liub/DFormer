...
**当前状态：方向级选择、A1 v1/v2 脚手架、A2 v1 训练接入与 v1 本地 GPU 单步 preflight、v2 协议修订、以及 v2 gradient-path isolation 门禁均已处置。`MMFR-A2-train-integration-v1` 已在正式 500 epoch 尚未执行的时点归档为 `superseded-before-formal-training`，当前身份为 `MMFR-A2-train-integration-v2`；`MMFR-A2-v2-gradient-path-isolation` 已于 2026-09-15 01:32 实际执行并通过，不再阻塞正式训练。正式训练职责已固定为云端单卡，本机只做推理和小规模 preflight；RTX 4090 是默认省钱卡，RTX 5090 只在 4090 缺货、24 GiB 不足或配对 probe 证明单位样本成本更低时使用。当前真正开放的是：R1/S1 补充协议中起草期自拟数值是否确认、是否允许 corruption 在原生无效 Depth 像素上产生非零输入、`extra_norms` 六个张量初始化的确定性，以及是否授权云端容量/吞吐 probe 与两个公平对照训练。**
...
- **仍开放：** 需要用户明确处置的项：(1) R1/S1/C1 模板中起草期自拟的数值（R1 的 Spearman 阈值、AURC/Brier 相对改进阈值、Holm-Bonferroni 家族大小与 α、bootstrap 重采样数、coverage 点数；S1 的连续退化容忍度与允许违反次数；C1 的 bootstrap 重采样数）目前都标为 `provisional-default-not-frozen-requires-user-confirmation`，是否确认或替换需要用户决定；(2) 是否允许 `gaussian_noise` 与 `misalignment` 在原生无效 Depth 像素上产生非零输入（代码中已分开记录为 `newly_valid_pixels`，当前实现保留 corruption 的原始语义，若要改为在原生无效区重新置零必须作为新口径决定并写入 protocol）；(3) `extra_norms.0/1/2.{weight,bias}` 这 6 个张量不由官方 pretrained 提供，其初始化在冻结 seed 下是否完全确定尚待核验；(4) 是否授权云端 batch size 10 的容量/吞吐 probe，以及是否随后启动 `MMFR-A2-clean-control-v2` 与 `MMFR-A2-depth-corruption-train-v2` 两个公平对照训练；(5) C1、R1、S1 与 `MMFR-B1-learned-geometry-adapter-v1` 各自都需要独立授权，B1 尚未细化，不因任何 preflight 或门禁通过自动解锁。
