...
**当前状态：方向级选择、A1 v1/v2 脚手架、A2 v1 训练接入与 v1 本地 GPU 单步 preflight、v2 协议修订、以及第二轮要求的本地资格门禁均已处置。`MMFR-A2-train-integration-v1` 已在正式 500 epoch 尚未执行的时点归档为 `superseded-before-formal-training`，当前身份为 `MMFR-A2-train-integration-v2`。已通过的本地门禁：`MMFR-A2-v2-initial-state-equivalence`、`MMFR-A2-v2-gradient-path-isolation`、`MMFR-A2-v2-amp-update-path-isolation`、可复跑 `MMFR-A2-v2-cpu-qualification`（74 项断言）；已完成的事实审计：`train-dev` Depth 有效性分布、validity transport。当前状态标识为 `local-qualifications-passed-awaiting-senior-review-of-dataset-validity-audit`：**尚未**由高级模型确认“数据集有效性审计不需要改变监督语义”，也尚未完成 source identity 冻结，因此还不能升级为 `eligible-for-cloud-capacity-probe-authorization`。正式训练职责仍固定为云端单卡、默认 RTX 4090，本机只做推理与本地资格检查。**
...
- **仍开放（第二轮更新版）：** 需要用户或高级模型处置的项：(1) **validity transport 的 A/B 选择**——A 为“原生无效 Depth 是吸收哨兵，`gaussian_noise`/`blur`/`quantization` 只能在已有有效像素上作用”，B 为“允许产生 spurious depth，但必须显式命名为 spurious-measurement corruption”；另有独立子问题：misalignment 下 validity 是否应随 Depth 一起做空间搬运（MID-A 随内容搬运 / MID-B 保持按目标坐标取值，当前实现是 MID-B）。实测依据见 validity transport 审计：噪声在最稀疏样本上单个 crop 可制造 `129,563`（severity 0.25）至 `132,929`（1.0）个非零的原本无效像素。(2) **43 个参数不在任何 optimizer param group**（`29` 个 `Geo.weight`、`8` 个 `patch_embed.proj.*`、`6` 个 `downsample.norm.*`），因此从不更新；两个身份同样受影响，是否处置需用户决定，任何修复都会改变训练语义、必须新建 protocol。(3) 是否授权云端 batch size 10 容量/吞吐 probe 与两个公平对照训练——**前置条件尚未全部满足**（缺高级模型对数据集有效性审计的确认与 source identity 冻结）。(4) C1、R1、S1 与 `MMFR-B1-learned-geometry-adapter-v1` 各自需要独立授权；B1 尚未细化。(5) `train-dev` 中 `8` 条样本在 `valid_mask` 内无有效 Depth、`27` 条有效率 `<1%`，是否需要在训练口径上特殊处理（当前口径不动）。
...
- **证据入口：** `doc/plans/2026-09-MUSeg-多形式模态失效可靠性学习/00-总方向规划.md`、`01-新对话最小上下文与当前任务.md`、`02-MMFR-A1失效基函数与可靠性脚手架.md`、`03-MMFR-A2训练接入与公平对照协议.md`；v1 归档模板 `protocols/mmfr-a2-train-integration-v1.template.json`、当前 v2 模板 `protocols/mmfr-a2-train-integration-v2.template.json` 与三个补充协议模板 `protocols/mmfr-r1-reliability-supplemental-v1.template.json`、`protocols/mmfr-s1-severity-sweep-v1.template.json`、`protocols/mmfr-c1-paper-confirmation-v1.template.json`；审计工具 `tools/mmfr/severity_burden_audit.py`（已运行通过，报告 SHA-256 `86e468c040648a790675456db1b828a5a71e560b849c75b4b7e9a3ca926c2f52`）与 `tools/mmfr/gradient_path_isolation.py`（未执行）；审计叙事 `liu-test-exp/方案1/MMFR-多形式模态失效可靠性学习-独立审计总规划.md`。

**大白话说明：** 协议已经在结果产生前改成修正版，旧协议连同它那次本地 GPU 更新一起归档，新身份另起；损坏强度的定义、分辨率无关性、真实缺失深度的处理、以及只在深度通道上算可靠性损失这四件事都改好了，强度审计已经跑通。最后那道门禁也已经真实跑过并通过：打开可靠性头完全不改变分割网络的 714 个参数梯度，所以 A2 阶段未来任何分割变化都不能归因于可靠性头，也就不需要再单独烧一次 500 epoch 的“只损坏、不加头”对照。现在卡点不再是数学前提，而是正式训练本身仍未授权，以及几项我们自己拟的数字和一个“是否允许损坏制造出假深度”的口径问题需要用户点头。
