...
> **状态时间：** 2026-09-24（MMFR v4.1 E1 Batch 1A C0/F-lite training PASS；Quick-Val 完成且 F-lite `promote`；十条件 Main-Val 已在本地完成，十视图口径下 F-lite 未复现 Quick-Val 优势、Batch 1A Main-Val 等待上级处置裁决；Batch 1B R-OE-lite Gate-B PASS、正式训练已于 2026-09-24 获授权并于 01:40:43 UTC 在单 GPU `screen` 中启动；截至 epoch 1 iter 84 为 `84/2560` successful updates，SwanLab run `https://swanlab.cn/@Newton_liub/DFormer-liu/runs/er8ao0f2` 已创建，loss finite、无 OOM/NaN，训练继续）。
...
**当前状态：Batch 1A 与 Batch 1B 的工程/评价状态分别登记。** Batch 1A C0/F-lite 的十条件 Main-Val 已完成，两侧各 10/10 条件，身份与配对性断言 0 失败；主指标 $M_6$ 为 C0 `55.2883`、F-lite `54.8917`（$\Delta_F=-0.3966$ pp），clean 为 `56.69` / `56.02`。该批没有 Main-Val 数值门槛，仍待上级独立裁决。Batch 1B R-OE-lite 已完成实现与 Gate-B（`PASS`、`failed_checks=[]`）；用户已于 2026-09-24 授权正式训练及成功后四条件 Quick-Val；纠正 `LOCAL_RANK=0` 并移除不完整的 CLI metadata 参数组后，正式训练于 01:40:43 UTC 启动，截至 epoch 1 iter 84 为 `84/2560` successful updates，SwanLab online run 已创建，暂无最终训练或评价结果。R-EM-lite 保持 `retired-by-observability`。Batch 1B 报告：`MMFR/02_evidence/report_e1_batch1b_roe_gateb.md`。
...
- **授权边界与恢复点：** Batch 1A C0/F-lite 正式训练、4-condition Quick-Val 与 10-condition Main-Val 均已执行完毕，Quick-Val 判定 F-lite `promote`，十视图 Main-Val 给出描述性对照结果（$\Delta_F=-0.3966$ pp）且无预注册门禁。R-OE-lite implementation 与 Gate-B 已完成并 PASS；正式训练及其成功后四条件 Quick-Val 已于 2026-09-24 获授权；修正启动环境后正式训练已运行至 `84/2560` successful updates，暂无 NaN/OOM，Quick-Val 尚未开始；训练期间搜索匹配 C0 checkpoint。十条件 Main-Val、Batch 2、T 与 official test 仍未授权，official test 保持 `sealed_unread`；Batch 1A Main-Val 结果的处置继续独立待上级裁决。

**当前恢复点：** `R-OE-formal-training-running`。云端单 GPU正式训练已在 `LOCAL_RANK=0` 环境下启动，screen `14333.mmfr-e1-batch1b-roe`、PID `14335`，截至 epoch 1 iter 84 为 `84/2560` successful updates；保持既有合同继续至 2560，不改参数。训练成功后按已授权范围直接运行既有四条件 Quick-Val。Batch 1A C0 fixed-final checkpoint 尚未在已查位置定位，搜索继续；Quick-Val 前必须确认可读。十条件 Main-Val、Batch 2、T 与 official test 仍未授权；official test 保持 `sealed_unread`。Batch 1A 十条件 Main-Val 的上级处置仍独立待审。其冻结协议和 Gate-B 报告位于 `MMFR/01_research/e1_batch1_protocol.md` 与 `MMFR/02_evidence/report_e1_batch1b_roe_gateb.md`。