...
- **授权边界与恢复点：** Batch 1A C0/F-lite 正式训练与 4-condition Quick-Val 均已执行完毕，Quick-Val 判定 F-lite `promote`。Main-Val 尚未运行，计划迁回本地；R-OE 实现、Batch 1B、Batch 2 与 official test 继续未运行，official test 保持 `sealed_unread`。

**下一恢复点：** `awaiting local 10-condition Main-Val`。Main-Val 必须在本地按冻结 evaluator、原始 Label grid、318 条 `val-dev` 和 10 views 运行；在此之前不形成 Main-Val 结论，也不启动 R-OE、T、Batch 1B、Batch 2 或 official test。