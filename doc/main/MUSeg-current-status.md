...
> **事实截至：** 2026-09-24 R-OE-lite v2 本地提交并推送、云端实例控制面只读核验；GPU 三步验证及新正式训练尚未开始。
...
1. 等用户手动启动 `cpod-1vbh7faqcauq` 并通知；在此之前不由代理启动实例、连接云端或运行 GPU 验证。v2 commit `255780f` 已在 `origin/perf/mmfr-a2-v3-pipeline-opt1`，本地状态文件这次的等待说明尚未另行提交或推送。
2. 用户通知实例已启动后，先核对控制面状态、云端 pull 后的 commit、环境、source checkpoint、GPU 空闲及训练入口，再运行 3 次连续成功更新并记录规定的显存与形状证据；依据条件授权决定正式重训或停止。
