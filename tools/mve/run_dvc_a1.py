...
from tools.mve.dvc_a1_protocol import DvcProtocol, DvcProtocolError, file_sha256, load_dvc_protocol
...
def _read_sample(protocol: DvcProtocol, sample_id: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    label[label < 0] = 255
    return rgb, depth16, depth8, label
...
def _verify_protocol_assets(protocol: DvcProtocol) -> list[str]:
    ...
    entries = split_entries(protocol.split_path)
    if len(entries) != int(protocol.raw["split"]["sample_count"]):
    ...
    if len(groups) != int(protocol.raw["split"]["location_group_count"]):
...
def _infer_condition(
    *,
    ...
    for sample_index, entry in enumerate(entries):
        sample_id = Path(entry).stem
        rgb, depth16, _, label = _read_sample(protocol, sample_id)
        masks = build_corruption_masks(
            depth16,
            sample_id,
            seed=int(protocol.raw["corruption"]["seed"]),
    ...
    return {
        "schema_version": "museg-dvc-a1-condition-result-v1",
...
def run_preflight(protocol: DvcProtocol, *, device: torch.device, sample_count: int = 2) -> dict[str, Any]:
    ...
    selected = entries[:sample_count]
    checks: list[dict[str, Any]] = []
    for entry in selected:
        sample_id = Path(entry).stem
        _, depth16, depth8, _ = _read_sample(protocol, sample_id)
        first = build_corruption_masks(depth16, sample_id)
        second = build_corruption_masks(depth16, sample_id)
        checks.append(
            {
                "sample_id": sample_id,
    ...
    if not all(all(value for key, value in row.items() if key != "sample_id") for row in checks):
    ...
    for entry in selected:
        sample_id = Path(entry).stem
        rgb, depth16, _, label = _read_sample(protocol, sample_id)
        masks = build_corruption_masks(depth16, sample_id)
    ...
    report = {
        "schema_version": "museg-dvc-a1-preflight-v1",
        "protocol_id": protocol.raw["protocol_id"],
        "protocol_sha256": protocol.sha256,
        "status": "passed",
        "generated_at_utc": _utc_now(),
...
def run_full(protocol: DvcProtocol, *, device: torch.device) -> dict[str, Any]:
    ...
    decision = adjudicate(dose, specificity, group_mines)
    summary = {
        "schema_version": "museg-dvc-a1-summary-v1",
        "protocol_id": protocol.raw["protocol_id"],
...
def _write_execution_record(
    ...
    *,
    ...
    record: dict[str, Any] = {
        "schema_version": "museg-dvc-a1-execution-v1",
        "protocol_id": protocol.raw["protocol_id"],
        "protocol_sha256": protocol.sha256,
        "mode": mode,
        "argv": list(argv),
    ...
    try:
        ...
        if protocol is not None:
            ...
            _atomic_write_json(
                output,
                {
                    "schema_version": "museg-dvc-a1-failure-v1",
                    "mode": args.mode,
